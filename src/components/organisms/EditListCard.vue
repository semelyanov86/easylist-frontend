<template>
    <v-card>
        <atom-card-title>{{ $t('lists.create-title') }}</atom-card-title>
        <edit-list-form
            @save-list="saveList"
...
import EditListForm from '@/components/molecules/EditListForm.vue'
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        function saveList(listModel: ListInterface) {
            emit('saveList', listModel)
        }
        function closeList() {
            emit('closeList')
        }
        return { saveList, closeList }
    },
})
</script>
